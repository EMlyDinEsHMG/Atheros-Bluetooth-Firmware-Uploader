...
//Variable to Monitor the upload
bool uploaded = false;
...
{
    IOLog("Atheros Bluetooth Firmware Uploader by mac4mat, modified by EMlyDinEsHMG (c) 2012\n");
...
{
    DEBUG_LOG("%s(%p)::message %d\n", getName(), this,type);
    switch ( type )
    {
        case kIOMessageServiceIsTerminated:
            if (pUsbDev->isOpen(this))
            {
                DEBUG_LOG("%s(%p)::message - service is terminated - closing device\n", getName(), this);
//                pUsbDev->close(this);
            }
            break;
        ...
        case kIOMessageServiceWasClosed: 
            DEBUG_LOG("%s(%p)::message - Service Closed\n", getName(), this);
            uploaded = false;
            break;
